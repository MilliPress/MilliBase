...
| `input_schema`        | no       | JSON Schema describing expected input. Core validates input against this schema *before* invoking the callback. When omitted, MilliBase defaults to `{'type' => 'object', 'additionalProperties' => false}` — "no input accepted" — so `wp ability run <id>` works without forcing every no-input ability to declare a schema. Abilities that *do* accept input must define their own schema explicitly. |
